...
def require_args():
    ...
    cfg.add_argument('--net-grayscale', default=False, type=eval,
                     help='whether to use grayscale preprocessing in the network')
    cfg.add_argument('--net-freeze', default=[], type=str, nargs='*',
                     help='name of parameters to be freeze')


def get(cin=None, cout=None, sobel=None, grayscale=None, frozen=None, **kwargs):
    ...
    grayscale = grayscale if grayscale is not None else cfg.net_grayscale
    assert (not (sobel and grayscale)), "Can't have sobel and grayscale enabled at the same time"
    assert (not sobel or cin == 3), ("Sobel descriptor requires cin equal "
                                     "to 3 but got [%d]" % cin)
    logger.debug('Backbone [%s] is declared with cin [%d] and cout [%d] [%s] sobel and [%s] grayscale'
                 % (cfg.get(_MODULE_), cin, cout, 'with' if sobel else 'without', 'with' if grayscale else 'without'))
    frozen = frozen if frozen is not None else cfg.net_freeze
    network = cfg.get_class(_MODULE_, cfg.get(_MODULE_))(cin, cout, sobel, grayscale, **kwargs)
